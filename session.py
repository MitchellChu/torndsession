...
class SessionManager(object):
    ...
    def __init_session_driver(self):
        ...
        if cache_driver:
            ...
            if not hasattr(cache_name, cache_handler):
                setattr(cache_name, cache_handler, SessionDriverFactory.create_driver(driver, driver_settings))
            session_driver = getattr(cache_name, cache_handler)
        else:
            session_driver = SessionDriverFactory.create_driver(driver, driver_settings)
        self.driver = session_driver() # create session driver instance.

    def __get_session_driver():
        cache_driver = self.settings.get("cache_driver", True)
        if cache_driver:
            if not hasattr('__cached_session_driver', self.handler.application):
                setattr('__cached_session_driver', self.handler.application, SessionDriverFactory.create_driver(driver_name, driver_settings))
            driver = getattr('__cached_session_driver', self.hanlder.application)
        else:
